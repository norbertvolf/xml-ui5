...
Promise.all([git.revparse(["--abbrev-ref", "HEAD"]), fs.readFile(commitMessageFileName)])
    .then(results => {
        let currentBranchName = results[0];
        let commitMessage = results[1].toString();
        let topic;
        let type;
        let matchCurrentBranch = currentBranchName.match(/^(feature|fix|internal)\/[^\/]+\/([^\/]+)/);
        ...
        if (matchCurrentBranch && !commitMessage.match(/^\[/)) {
            ...
            commitMessage = `[${topic}] ${commitMessage}\n\nIssue Type: ${type.toUpperCase()}\nJIRA: ${topic}`;
            messageUpdated = true;
